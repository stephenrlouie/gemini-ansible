...
*This document is oriented at users who want a deeper understanding of the Gemini
API structure, and developers wanting to extend the Gemini API.*
...
The conventions of the [Gemini API](../api.md) (and related APIs in the ecosystem) are intended to ease client development and ensure that configuration mechanisms can be implemented that work across a diverse set of use cases consistently.

The general style of the Gemini API is RESTful - clients create, update, delete, or retrieve a description of an object via the standard HTTP verbs (POST, PUT, DELETE, and GET) - and those APIs preferentially accept and return JSON. Gemini also exposes additional endpoints for non-standard verbs and allows alternative content types. All of the JSON accepted and returned by the server has a schema, identified by the "kind" and "apiVersion" fields. Where relevant HTTP header fields exist, they should mirror the content of JSON fields, but the information should not be represented only in the HTTP header.
...
Each resource typically accepts and returns data of a single kind.  A kind may be accepted or returned by multiple resources that reflect specific use cases. For instance, the kind "Cluster" is exposed as a "clusters" resource that allows end users to create, update, and delete clusters, while a separate "cluster status" resource (that acts on "Cluster" kind) allows automated processes to update a subset of the fields in that resource.
...
1. **Objects** represent a persistent entity in the system.
   ...
   Examples: `Project`, `Cluster`, `ReplicationController`.
...
2. **Lists** are collections of **resources** of one (usually) or more (occasionally) kinds.
   ...
   In addition, all lists that return objects with [labels](http://gemini-project.github.io/docs/user-guide/labels/) should support label filtering, and most lists should support filtering by fields.

   Examples: ClusterLists, ProjectLists
...
3. **Simple** kinds are used for specific actions on objects and for non-persistent entities.
   ...
   * `/binding`: Used to bind a resource representing a user request (e.g., Cluster, PersistentVolumeClaim) to a cluster infrastructure resource (e.g., Node, PersistentVolume).
   * `/status`: Used to write just the status portion of a resource. For example, the `/clusters` endpoint only allows updates to `metadata` and `spec`, since those reflect end-user intent. An automated process should be able to modify status for users to see by sending an updated Cluster kind to the server to the "/clusters/&lt;name&gt;/status" endpoint - the alternate endpoint allows different rules to be applied to the update, and access to be appropriately restricted.
...
References to loosely coupled sets of objects, such as [clusters](../user-guide/clusters.md) overseen by a [replication controller](../user-guide/replication-controller.md), are usually best referred to using a [label selector](../user-guide/labels.md). In order to ensure that GETs of individual objects remain bounded in time and space, these sets may be queried via separate API queries, but will not be expanded in the referring object's status.
...
There are no maps of subobjects in any API objects. Instead, the convention is to use a list of subobjects containing name fields.
...
Sometimes, at most one of a set of fields can be set.  For example, the [volumes] field of a ClusterSpec has 17 different volume type-specific
...
An example is a Service, which represents the intent of the user to group a set of clusters with common behavior on common ports. When Gemini detects a cluster matches the service selector, the IP address and port of the cluster are added to an Endpoints resource for that Service. The Endpoints resource exists only if the Service exists, but exposes only the IPs and ports of the selected clusters.  The full service is represented by two distinct resources - under the original Service resource the user created, as well as in the Endpoints resource.

As another example, a "cluster status" resource may accept a PUT with the "cluster" kind, with different rules about what fields may be changed.

Future versions of Gemini may allow alternative encodings of objects beyond JSON.
...
* GET /&lt;resourceNamePlural&gt; - Retrieve a list of type &lt;resourceName&gt;, e.g. GET /clusters returns a list of Clusters.
* POST /&lt;resourceNamePlural&gt; - Create a new resource from the JSON object provided by the client.
* GET /&lt;resourceNamePlural&gt;/&lt;name&gt; - Retrieves a single resource with the given name, e.g. GET /clusters/first returns a Cluster named 'first'. Should be constant time, and the resource should be bounded in size.
...
In the standard JSON merge patch, JSON objects are always merged but lists are always replaced. Often that isn't what we want. Let's say we start with the following Cluster:
...
...and we POST that to the server (as JSON). Then let's say we want to *add* nodes to the cluster.

```yaml
PATCH /api/v1/namespaces/default/clusters/cluster1
spec:
  type: kubernetes
    - name: cluster1
      nodes: 8
...
If we were to use standard Merge Patch, the existing 5 node list would be replaced with 8 new nodes. However, our intent is for the node lists to merge together based on the `name` field.

To solve this problem, Strategic Merge Patch uses metadata attached to the API objects to determine what lists should be merged and which ones should not. Currently the metadata is available as struct tags on the API objects themselves, but will become available to clients as Swagger annotations in the future. In the above example, the `patchStrategy` metadata for the `nodes` field would be `merge` and the `patchMergeKey` would be `name`.
...
To override the node list to be strictly replaced, regardless of the default:
...
clusters:
  - name: cluster1
    nodes: 5
  - $patch: delete
    name: 8  # merge key and value goes here
```
...
All compatible Gemini APIs MUST support "name idempotency" and respond with an HTTP status code 409 when a request is made to POST an object that has the same name as an existing object in the system. See [identifiers](http://gemini-project.github.io/docs/user-guide/identifiers/) for details.
...
For example, the scheduler sets the `cluster.spec.nodeName` field after the cluster is created.
...
Gemini leverages the concept of *resource versions* to achieve optimistic concurrency. All Gemini resources have a "resourceVersion" field as part of their metadata. This resourceVersion is a string that identifies the internal version of an object that can be used by clients to determine when objects have changed. When a record is about to be updated, it's version is checked against a pre-saved value, and if it doesn't match, the update fails with a StatusConflict (HTTP status code 409).
...
The resourceVersion is currently backed by [etcd's modifiedIndex](https://coreos.com/docs/distributed-configuration/etcd-api/). However, it's important to note that the application should *not* rely on the implementation details of the versioning system maintained by Gemini. We may change the implementation of resourceVersion in the future, such as to change it to a timestamp or per-object counter.
...
Gemini will always return the `Status` kind from any API endpoint when an error occurs.
...
```console
$ curl -v -k -H "Authorization: Bearer WhCDvq4VPpYhrcfmF6ei7V9qlbqTubUc" https://10.240.122.184:443/api/v1/namespaces/default/clusters/grafana

> GET /api/v1/namespaces/default/clusters/grafana HTTP/1.1
...
< 
{
  ...
  "metadata": {},
  "status": "Failure",
  "message": "clusters \"grafana\" not found",
  "reason": "NotFound",
  "details": {
    "name": "grafana",
    "kind": "clusters"
...
However, we should support conveniences for common cases by default. For example, what we now do in ReplicationController is automatically set the RC's selector and labels to the labels in the cluster template by default, if they are not already set. That ensures that the selector will match the template, and that the RC can be managed using the same labels as the clusters it creates. Note that once we generalize selectors, it won't necessarily be possible to unambiguously generate labels that match an arbitrary selector.

If the user wants to apply additional labels to the clusters that it doesn't select upon, such as to facilitate adoption of clusters or in the expectation that some label values will change, they can set the selector to a subset of the cluster labels. Similarly, the RC's labels could be initialized to a subset of the cluster template's labels, or could include additional/different labels.
...
In fact, in-development API fields, including those used to represent fields of newer alpha/beta API versions in the older stable storage version, may be represented as annotations with the form `something.alpha.gemini.io/name` or `something.beta.gemini.io/name` (depending on our confidence in it).  For example `net.alpha.gemini.io/policy` might represent an experimental network policy field.  The "name" portion of the annotation should follow the below conventions for annotations.  When an annotation gets promoted to a field, the name transformation should then be mechanical: `foo-bar` becomes `fooBar`.

Other advice regarding use of labels, annotations, and other generic map keys by Gemini components and tools:
  - Key names should be all lowercase, with words separated by dashes, such as `desired-replicas`
  - Prefix the key with `gemini.io/` or `foo.gemini.io/`, preferably the latter if the label/annotation is specific to `foo`
    - For instance, prefer `service-account.gemini.io/name` over `gemini.io/service-account.name`
...
[![Analytics](https://gemini-site.appspot.com/UA-36037335-10/GitHub/docs/devel/api-conventions.md?pixel)]()
